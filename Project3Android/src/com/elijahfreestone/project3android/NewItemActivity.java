...
public class NewItemActivity extends Activity {
		...
		saveButton.setOnClickListener(new OnClickListener() {   
			...
			public void onClick(View arg0) { 
				...
				if (ConnectionManager.connectionStatus(myContext)) {
					//Validate fields and save item
					saveItemToParse();
				} else {
					//Alert user of no connection
					noConnectionAlert();
				}
			} 
	...
	void saveItemToParse() {
		...
		//Make sure formatted number is 13 long "(xxx)xxx-xxxx" and doesn't start with 0 or 1
		} else if (numberEnteredString.length() != 13 || firstInt <= 1) { 
			//Create alert message string and show error alert
			...
			if (itemName.equalsIgnoreCase("none")) {
				...
				//Intent had data so this is an edit 
				//Log.i(TAG, "itemName = " + itemName);
				//Query Parse to grab item being edited
	...
	public static void noConnectionAlert() {
		...
		// version so getString must be used.
		alertDialog
				.setMessage("No Network detected so the contact was not saved. "
